...
void Adafruit_ST7789::setRotation(uint8_t m) {
  ...
  case 3:
    ...
    _width = windowHeight;
    break;
  case 4: // Corrige espelhamento (mirror)
    madctl =  ST77XX_MADCTL_MX | ST77XX_MADCTL_RGB;
    _xstart = _colstart;
    _ystart = _rowstart2;
    _height = windowHeight;   // altura = largura física
    _width = windowWidth;   // largura = altura física
    break;
  case 5: // Corrige espelhamento (mirror)
    madctl = ST77XX_MADCTL_MV | ST77XX_MADCTL_RGB;
    _xstart = _colstart;
    _ystart = _rowstart2;
    _height = windowWidth;   // altura = largura física
    _width = windowHeight;   // largura = altura física
    break;
  }
